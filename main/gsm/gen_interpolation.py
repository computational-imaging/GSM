...
def generate_one_video( z_seeds, G, c, output_dir, truncation_cutoff=14, truncation_psi=0.7):
    device = c.device

    lo, hi = [-1, 1]
    len_seeds = len(z_seeds)
    for seed1, seed2 in zip(z_seeds[:len_seeds//2], z_seeds[(len_seeds//2):]):
        video_output_dir = os.path.join(output_dir, os.path.splitext(os.path.basename(str(seed1) + '__' + str(seed2)))[0])
        ...
        ws1 = G.mapping(z1, c, truncation_cutoff=truncation_cutoff, truncation_psi=truncation_psi)
