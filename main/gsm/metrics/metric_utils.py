...
class MetricOptions:
    def __init__(self, G=None, G_kwargs={}, dataset_kwargs={}, num_gpus=1, rank=0, device=None, progress=None, cache=True, verbose=False, output_dir="./"):
        ...
        if verbose and not self.output_dir:
            self.output_dir = "./"
...
def compute_feature_stats_for_dataset(opts, detector_url, detector_kwargs, rel_lo=0, rel_hi=1, batch_size=64, data_loader_kwargs=None, max_items=None, **stats_kwargs):
    ...
    if opts.cache:
        ...
        # Load.
        if flag:
            print(f'find cache stats for this dataset: {cache_tag}')
    ...
    _counter = 0
    for images, mask, _labels in torch.utils.data.DataLoader(dataset=dataset, sampler=item_subset, batch_size=batch_size, **data_loader_kwargs):
        if images.shape[1] == 1:
            images = images.repeat([1, 3, 1, 1])
        if _counter < 10 and opts.verbose and opts.rank == 0:
            save_image(images / 255.0, os.path.join(opts.output_dir, f'reals_{_counter:03d}.png'), normalize=False)
            _counter += images.shape[0]
...
def compute_feature_stats_for_generator(opts, detector_url, detector_kwargs, rel_lo=0, rel_hi=1, batch_size=64, batch_gen=None, **stats_kwargs):
    ...
    while not stats.is_full():
        ...
        for _i in range(batch_size // batch_gen):
            ...
            if _counter < 10 and opts.verbose and opts.rank == 0:
                # save images
                save_image(img / 255.0, os.path.join(opts.output_dir, f'images_{_counter:03d}.png'), normalize=False)
                _counter += img.shape[0]
