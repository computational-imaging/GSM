...
from utils.graphics_utils import (focal2fov, fov2focal,
                                  getProjectionMatrixTensor, getWorld2View2)


def parse_raw_labels(label_arr):
    cam2world = label_arr[:, :16].reshape(-1,4,4)
...
def create_new_camera(parsed_labels, image_width, img_height, device):
    ...
    proj_matrix = getProjectionMatrixTensor(znear=0.01, zfar=100,
    ...
    cam = MiniCam(
        ...
        znear = 0.01,
        zfar = 100,
        world_view_transform=world_view_transform.transpose(-1,-2).to(device),
        full_proj_transform=(proj_matrix @ world_view_transform).transpose(-1,-2).to(device),
    )
    cam.projection_matrix = proj_matrix.tranpose(-1,-2).to(device)
    return cam
